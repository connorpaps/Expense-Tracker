...
describe('US7 vault isolation and safe copies', () => {
  it('keeps personal and demo vault records strictly isolated', async () => {
    ...
    try {
      ...
      expect((await listVaults(db)).map((vault) => vault.vault_owner_label)).toEqual([
        'Personal',
        'Portfolio demo',
      ]);
      ...
      expect(
        (await listTransactions(db, { vaultId: demo.id })).every(
          (transaction) => transaction.source_type === 'demo',
        ),
      ).toBe(true);
      expect(
        (
          await db.get<{ n: number }>(
            'SELECT COUNT(*) AS n FROM demo_datasets WHERE vault_id = ?',
            [demo.id],
          )
        )?.n,
      ).toBe(1);
  ...
  it('imports a verified backup as a new isolated copy without changing the source', async () => {
    ...
    try {
      ...
      if (!previousCategoryId)
        throw new Error('The demo vault should contain a category for remapping coverage.');
      ...
      if (!sourceTransactionId || !sourceTransaction.merchant_display)
        throw new Error('The demo vault should contain a transaction for remapping coverage.');
      await db.exec(
      ...
      const copyId = await importAsNewVault(
        db,
        await buildVaultExport(db, source.id),
        'Friend copy',
      );
      ...
      const sourceCategoryRows = await listCategories(db, source.id);
      const copiedCategoryRows = await listCategories(db, copyId);
      expect(copiedCategoryRows).toHaveLength(sourceCategoryRows.length);
      expect(sourceCategoryRows.some((category) => category.name === 'Subscriptions')).toBe(true);
      expect(copiedCategoryRows.some((category) => category.name === 'Subscriptions')).toBe(true);
      const copiedCorrection = await db.get<{
      ...
      const copiedTransaction = copiedTransactions.find(
        (transaction) => transaction.merchant_display === sourceTransaction.merchant_display,
      );
