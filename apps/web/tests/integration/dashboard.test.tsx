...
describe('US3 web dashboard', () => {
  it('shows exact totals, credits, category breakdown, and mixed-currency notice', async () => {
    render(
      <MemoryRouter>
        <DashboardPage db={fakeDb()} vaultId="vault-test" defaultCurrency="USD" />
      </MemoryRouter>,
    );
    ...
    expect(
      screen.getByText(
        'This period includes other currencies. Totals below show USD only; no conversion is applied.',
      ),
    ).toBeInTheDocument();
  ...
  it('switches to a custom range and exposes an accessible empty state', async () => {
    ...
    expect(screen.getByRole('status')).toHaveTextContent('Your vault has 3 saved records');
  ...
  it('explains when the selected period only contains another currency', async () => {
    const user = userEvent.setup();
    render(
      <MemoryRouter>
        <DashboardPage db={fakeDb()} vaultId="vault-test" defaultCurrency="USD" />
    ...
    await user.clear(to);
    await user.type(to, '2026-08-05');

    expect(
      await screen.findByRole('heading', { name: 'No USD activity in this period' }),
    ).toBeInTheDocument();
    expect(
      screen
        .getAllByRole('status')
        .some((element) => element.textContent?.includes('none use USD')),
    ).toBe(true);
