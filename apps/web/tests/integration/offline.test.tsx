...
function rows<T extends SqlRow>(value: T[]): T[] {
  return value;
}
  ...
  const db: Db = {
    async exec(sql: string, params: unknown[] = []) {
      if (sql.includes('INSERT OR IGNORE INTO mutation_log')) {
        mutations.push({
          ...
          entity_type: asString(params[2]),
          entity_id: asString(params[3]),
          operation: asString(params[4]),
          base_version: asNumber(params[5]),
          device_id: asString(params[6]),
          lamport_clock: asNumber(params[7]),
          vector_clock: asString(params[8]),
          changed_fields: asString(params[9]),
          ciphertext: asString(params[10]),
          origin: asString(params[11]),
          status: 'pending',
          conflict_id: null,
          created_at: asString(params[14]),
          applied_at: null,
          retry_count: 0,
        ...
        transactions.push({
          id: asString(id),
          vault_id: asString(vaultId),
          occurred_on: asString(occurredOn),
          merchant_display: asString(merchant),
          merchant_original: typeof original === 'string' ? original : null,
          amount_minor: asNumber(amount),
          currency: asString(currency),
          category_id: typeof categoryId === 'string' ? categoryId : null,
          category_source: typeof categorySource === 'string' ? categorySource : null,
          category_confidence: typeof categoryConfidence === 'string' ? categoryConfidence : null,
          note: typeof note === 'string' ? note : null,
          source_type: asString(sourceType),
          statement_import_id: typeof importId === 'string' ? importId : null,
          source_row_key: typeof sourceRowKey === 'string' ? sourceRowKey : null,
          review_state: asString(reviewState),
          original_payload: typeof originalPayload === 'string' ? originalPayload : null,
          ...
          last_modified_by: asString(modifiedBy),
        });
        ...
        if (transaction) {
          transaction.deleted_at = asString(params[0]);
        ...
        if (transaction) {
          ...
          if (sql.includes('merchant_display = ?'))
            transaction.merchant_display = asString(params[index++]);
          if (sql.includes('amount_minor = ?'))
            transaction.amount_minor = asNumber(params[index++]);
    ...
    async all<T extends SqlRow = SqlRow>(sql: string, params: unknown[] = []): Promise<T[]> {
      ...
      if (sql.includes('FROM transactions')) {
        ...
        return rows(
          transactions.filter(
            (transaction) =>
              transaction.vault_id === vaultId &&
              transaction.deleted_at === null &&
              (!start || asString(transaction.occurred_on) >= start) &&
              (!end || asString(transaction.occurred_on) <= end) &&
    ...
    async get<T extends SqlRow = SqlRow>(
      ...
      if (sql.includes('COUNT(*) AS n'))
        return {
          n: mutations.filter(
            (mutation) => mutation.vault_id === params[0] && mutation.status === 'pending',
          ).length,
        } as unknown as T;
      ...
      if (sql.includes('FROM categories'))
        return categories.find(
          (category) => category.vault_id === params[0] && category.id === params[1],
        ) as unknown as T | undefined;
      if (sql.includes('FROM mutation_log WHERE'))
        return mutations.find(
          (mutation) => mutation.vault_id === params[0] && mutation.id === params[1],
        ) as unknown as T | undefined;
      return undefined;
    },
...
describe('US5 web offline and remount persistence integration', () => {
  it('saves, edits, categorizes, summarizes, and remounts locally while offline', async () => {
    ...
    const firstView = render(
      <MemoryRouter>
        <TransactionsPage db={db} vaultId="vault-offline" defaultCurrency="USD" />
      </MemoryRouter>,
    );
    ...
    const correctionRows = await db.all<SqlRow>(
      'SELECT * FROM category_correction_history WHERE vault_id = ?',
      ['vault-offline'],
    );
    expect(correctionRows).toHaveLength(1);
    expect(correctionRows[0]).toMatchObject({
      next_category_id: 'category-other',
      previous_category_id: 'category-food',
    });
    firstView.unmount();
    ...
    const dashboardView = render(
      <MemoryRouter>
        <DashboardPage db={db} vaultId="vault-offline" defaultCurrency="USD" />
      </MemoryRouter>,
    );
