...
const { encryptMutationPayload, mutationEnvelopeContext } = vi.hoisted(() => ({
...
vi.mock('../../src/local', () => ({ encryptMutationPayload, mutationEnvelopeContext }));
...
function createSyncDb(): Db {
  ...
  const db: Db = {
    ...
    async get<T extends SqlRow = SqlRow>(
      ...
      if (sql.includes('FROM conflicts')) {
        return (params[0] === 'vault-sync' && params[1] === 'conflict-1' ? conflict : undefined) as
          T | undefined;
      }
      if (sql.includes('FROM mutation_log')) {
        return (
          params[0] === 'vault-sync' && params[1] === 'resolve-conflict-conflict-1'
            ? mutation
            : undefined
        ) as T | undefined;
      }
...
describe('US6 web local sync boundary', () => {
  ...
  it('lists opaque conflicts and resolves direct choices without decrypting candidates', async () => {
    ...
    render(
      <MemoryRouter>
        <SyncPage db={db} vaultId="vault-sync" />
      </MemoryRouter>,
    );
  ...
  it('encrypts valid manual JSON before saving a local resolution', async () => {
    ...
    fireEvent.change(editor, {
      target: { value: '{"amount_minor":1234,"note":"corrected locally"}' },
    });
    ...
    expect(encryptMutationPayload).toHaveBeenCalledWith(
      { amount_minor: 1234, note: 'corrected locally' },
      JSON.stringify({
        mutation_id: 'resolve-conflict-conflict-1',
        vault_id: 'vault-sync',
        entity_type: 'conflict',
        entity_id: 'conflict-1',
        operation: 'update',
        base_version: 0,
        changed_fields: ['amount_minor', 'note'],
      }),
    );
  });
  ...
  it('encrypts a valid keep-both payload before saving a local resolution', async () => {
    ...
    fireEvent.change(editor, {
      target: {
        value:
          '{"local":{"amount_minor":1234,"note":"local"},"remote":{"amount_minor":1250,"note":"remote"}}',
      },
    });
    ...
    expect(encryptMutationPayload).toHaveBeenCalledWith(
      {
        local: { amount_minor: 1234, note: 'local' },
        remote: { amount_minor: 1250, note: 'remote' },
      },
      JSON.stringify({
        mutation_id: 'resolve-conflict-conflict-1',
        vault_id: 'vault-sync',
  ...
  it('rejects unknown-field conflicts before encryption', async () => {
    ...
    db.all = async <T extends SqlRow = SqlRow>(sql: string) => {
      ...
      if (sql.includes('FROM conflicts'))
        return [{ ...rows[0], conflicting_fields: '["*"]' }] as unknown as T[];
      return rows;
    };
    render(
      <MemoryRouter>
  ...
  it('rejects malformed merged fields before encryption', async () => {
    ...
    fireEvent.change(editor, {
      target: {
        value:
          '{"local":{"amount_minor":"wrong","note":"local"},"remote":{"amount_minor":1250,"note":"remote"}}',
      },
    });
    await user.click(screen.getByRole('button', { name: 'Save keep both decision' }));

    expect(await screen.findByRole('alert')).toHaveTextContent(
      'amount_minor must be a non-zero integer',
    );
    expect(encryptMutationPayload).not.toHaveBeenCalled();
    expect(screen.getByText('1 open')).toBeInTheDocument();
  });
