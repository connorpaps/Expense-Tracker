...
import { recordCategoryCorrection, rememberMerchantRule, listCorrectionHistory } from '../src/categorization/personal-rules';
...
const rule = (id: string, category_id: string, matcher: string, overrides: Partial<CategorizationRule> = {}): CategorizationRule => ({
  id, vault_id: 'v1', category_id, rule_type: 'personal_merchant', matcher, priority: 10, confidence: 0.9, evidence_count: 1, is_active: true, created_from: 'user_correction', created_at: 'x', updated_at: 'x', version: 1, ...overrides,
});
...
describe('Category suggestions (US4)', () => {
  ...
  it('suggests default-rule categories with explainable provenance', () => {
    ...
    expect(suggestion).toMatchObject({ categoryId: 'cat-transport', source: 'default_rule', confidence: 'high', matchedPattern: 'uber' });
  ...
  it('gives the most specific personal rule precedence over a shorter rule', () => {
    const suggestion = suggestCategory('The Moon Cafe Downtown', {
      categories,
      personalRules: [rule('short', 'cat-food', 'moon', { priority: 50 }), rule('specific', 'cat-other', 'moon cafe downtown', { priority: 1 })],
    });
    expect(suggestion).toMatchObject({ categoryId: 'cat-other', source: 'personal_rule', matchedRuleId: 'specific', matchedPattern: 'moon cafe downtown' });
  });

  it('keeps equal-precedence conflicting rules reviewable', () => {
    const suggestion = suggestCategory('Shared Merchant', {
      categories,
      personalRules: [rule('a', 'cat-food', 'shared merchant'), rule('b', 'cat-transport', 'shared merchant')],
    });
  ...
  it('uses evidence to raise learned rules to confirmed confidence', () => {
    const suggestion = suggestCategory('Corner Cafe', { categories, personalRules: [rule('learned', 'cat-food', 'corner cafe', { evidence_count: 3 })] });
  ...
  it('persists correction history and strengthens one learned rule instead of duplicating it', async () => {
    await withNodeDb(async (db) => {
      ...
      await insertVault(db, { id: 'v1', vault_owner_label: null, default_currency: 'USD', locale: 'en-US', week_start: 'locale_default', demo_mode: false, created_at: 'x', updated_at: 'x', deleted_at: null });
      for (const category of categories) await insertCategory(db, category);
      await recordCategoryCorrection(db, { vaultId: 'v1', merchant: 'Corner Cafe #42', nextCategoryId: 'cat-food', previousCategoryId: 'cat-other', now: '2026-08-05T00:00:00.000Z' });
      const first = await rememberMerchantRule(db, { vaultId: 'v1', merchant: 'Corner Cafe #42', categoryId: 'cat-food', now: '2026-08-05T00:00:00.000Z' });
      const second = await rememberMerchantRule(db, { vaultId: 'v1', merchant: 'CORNER CAFE', categoryId: 'cat-food', now: '2026-08-05T00:01:00.000Z' });
