...
import { insertStatementImport, insertVault, getStatementImport, listImportRows, listTransactions, insertCategory, listRules } from '../src/storage/repository';
...
describe('Import commit planning (T035)', () => {
  ...
  it('persists the committed import, review rows, and accepted transactions atomically', async () => {
    await withNodeDb(async (db) => {
      ...
      expect((await findMutation(db, 'vault-1', 'import-commit-import-1'))?.ciphertext).toBe('test-encrypted-import');
    });
  });
  ...
  it('persists an explicit category correction, learned rule, and provenance atomically', async () => {
    await withNodeDb(async (db) => {
      await applySchema(db);
      await insertVault(db, { id: 'vault-categorization', vault_owner_label: null, default_currency: 'USD', locale: 'en-US', week_start: 'locale_default', demo_mode: false, created_at: '2026-08-05T00:00:00.000Z', updated_at: '2026-08-05T00:00:00.000Z', deleted_at: null });
      await insertCategory(db, { id: 'cat-food', vault_id: 'vault-categorization', name: 'Food and Dining', slug: 'food', kind: 'expense', color_token: 'copper', icon_name: 'utensils', position: 0, is_active: true, created_at: 'x', updated_at: 'x', version: 1 });
      await insertCategory(db, { id: 'cat-other', vault_id: 'vault-categorization', name: 'Other', slug: 'other', kind: 'other', color_token: 'stone', icon_name: 'ellipsis', position: 1, is_active: true, created_at: 'x', updated_at: 'x', version: 1 });
      const corrected = row('row-correction', { import_id: 'import-correction', parsed_merchant: 'Corner Cafe', suggested_category_id: 'cat-other', category_source: 'default_rule', category_confidence: 'medium', user_decision: 'accept' });
      await commitImportToDb(db, {
        session: { id: 'import-correction', vault_id: 'vault-categorization', file_name: 'statement.csv', file_type: 'csv', file_size_bytes: 10, source_fingerprint: 'correction', bank_profile: null, parser_version: 'test', status: 'review', total_rows: 1, recognized_rows: 1, warning_count: 0, error_count: 0, storage_reference: null, created_at: 'x', completed_at: null, deleted_at: null },
        rows: [corrected],
        categoryCorrections: [{ rowId: corrected.id, categoryId: 'cat-food', rememberRule: true }],
        now: '2026-08-05T00:00:00.000Z', mutationCiphertext: 'encrypted',
      });
